...
const props = defineProps<{
    camera: typeof Shared.Camera,
    room: typeof Shared.room,
    selectedTool: typeof Shared.ROOM_TOOL_TYPE,
    selectedEntity: typeof Shared.Instance_Base,
}>();
...
//data
const TOOL_ENUMS = Shared.ROOM_TOOL_TYPE;
...
const showInstProps = computed(()=>(
    ...
    props.selectedEntity.TYPE == Shared.ENTITY_TYPE.INSTANCE &&
    !showCameraProps &&
    !showRoomProps
));
const showCameraProps = computed(()=>props.selectedTool == Shared.ROOM_TOOL_TYPE.CAMERA);
const showExitProps = computed(()=>(
    props.selectedEntity &&
    props.selectedEntity.TYPE == Shared.ENTITY_TYPE.EXIT &&
    !showCameraProps &&
    !showRoomProps
));
const showRoomProps = computed(()=>props.selectedTool == Shared.ROOM_TOOL_TYPE.ROOM_PROPERTIES);
const showPlaceHolder = computed(()=>!(showInstProps || showCameraProps || showExitProps || showRoomProps));
const destinationRoomExits = computed<typeof Shared.Exit>(()=>{
    if (props.selectedEntity.TYPE == Shared.ENTITY_TYPE.EXIT){
        const allRooms = gameDataStore.getAllRooms;
        const destRoom = allRooms.find(r => r.id == props.selectedEntity.destinationRoom);

        destRoom?.getAllExits();
    }
...
function setFollowObj(): void {
    if (props.selectedEntity.TYPE == Shared.ENTITY_TYPE.INSTANCE){
        props.camera.followObjId = props.selectedEntity.id;
    }
}

function setRoomBgColor(color: typeof Shared.Color): void {
    if (bgColorBtn.value){
    ...
    <div class="properties">
        <div v-if="showInstProps" class="propContents">
            ...
            <div class="control">
                ...
                <select id="instCollisionOvrr" :value="selectedEntity.collisionOverride" v-tooltip="$t('room_editor.tt_coll_ovr')"
            ...
            <div class="control">
                ...
                <input id="instCustDepth" type="number" :value="selectedEntity.zDepthOverride" v-tooltip="$t('room_editor.tt_cust_depth')"
        ...
        <div v-if="showCameraProps" class="propContents">
            ...
            <div v-show="camera.moveType == Shared.Camera.MOVE_TYPES.SCROLL" class="control">
                ...
                <select id="camScrollDir" :value="camera.scrollDir" v-tooltip="$t('room_editor.tt_camera_scroll_dir')"
                    ...
                    <option :value="Shared.Camera.SCROLL_DIRS.RIGHT">{{$t('room_editor.right')}}</option>
                </select>
            </div>
            <div v-show="camera.moveType == Shared.Camera.MOVE_TYPES.SCROLL" class="control">
        ...
        <div v-if="showExitProps" class="propContents">
            ...
            <div class="control">
                ...
                <select id="exitTrans" :value="selectedEntity.transition" v-tooltip="$t('room_editor.tt_exit_trans')"
                    @change="setExitProp({transition: ($event.target! as AnyObj).value})">
                    <option :value="Shared.Exit.TRANSITION_TYPES.NONE">{{$t('generic.no_option')}}</option>
                    <option :value="Shared.Exit.TRANSITION_TYPES.FADE">{{$t('room_editor.trans_fade')}}</option>
                </select>
            </div>
            <div class="control">
                <label for="exitEnding">{{$t('room_editor.is_ending')}}:</label>
                <input id="exitEnding" type="checkbox" :checked="selectedEntity.isEnding" v-tooltip="$t('room_editor.tt_exit_is_ending')"
            ...
            <div v-show="!selectedEntity.isEnding" class="control">
                ...
                <select id="exitDestRoom" :value="selectedEntity.destinationRoom" v-tooltip="$t('room_editor.tt_exit_dest_room')"
            ...
            <div v-show="!selectedEntity.isEnding && selectedEntity.destinationRoom != null" class="control">
                ...
                <select id="exitDestExit" :value="selectedEntity.destinationExit" v-tooltip="$t('room_editor.tt_exit_dest_exit')"
            ...
            <div v-show="selectedEntity.isEnding" class="control">
                ...
                <textarea id="endingDialog" :value="selectedEntity.endingDialog" v-tooltip="$t('room_editor.tt_exit_end_dialog')"
